...
// Helper to extract products from API response
const extractProducts = (response) => {
  if (Array.isArray(response)) return response;
  if (response && response.success && Array.isArray(response.products)) return response.products;
...
const normalizeCategory = (cat) => {
  if (!cat) return '';
  return cat.toString().toLowerCase().replace(/\s+/g, '').replace(/-/g, '');
};
...
const Products = () => {
  ...
  useEffect(() => {
    ...
    if (category && validCategories.includes(category)) {
      ...
      if (!initialLoad) fetchProducts(category);
    } else {
      setSelectedCategory('all');
      if (!initialLoad) fetchProducts('all');
    }
  ...
  const fetchProducts = async (category = selectedCategory) => {
    try {
      ...
      if (category !== 'all') {
        ...
        productsData = extractProducts(response);
        
        // Fallback to client-side filtering if backend returns empty
        if (productsData.length === 0) {
          const allResponse = await productAPI.getAllProducts({ _t: lastUpdate });
          ...
          productsData = allProducts.filter(product => 
            normalizeCategory(product.category) === normalizedTarget
          );
        }
      } else {
        response = await productAPI.getAllProducts({ _t: lastUpdate });
        productsData = extractProducts(response);
      }
      ...
      if (category === 'all' || initialLoad) {
        ...
        categories.forEach(cat => {
          const normalizedCat = normalizeCategory(cat.value);
          counts[cat.value] = allProducts.filter(p => 
            normalizeCategory(p.category) === normalizedCat
          ).length;
        });
  ...
  // Initial load
  useEffect(() => {
    if (initialLoad) fetchProducts(selectedCategory);
  }, []);
  ...
  const getCategoryInfo = (value) => categories.find(cat => cat.value === value);
  ...
  const getProductsToShow = () => {
    ...
    if (isMobile) return showAllProducts ? filteredProducts : filteredProducts.slice(0, 4);
              ...
              <div className="flex items-center flex-wrap gap-3">
                ...
                {selectedCategory !== 'all' && (
                  <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-700 rounded-full text-sm font-medium">
                    ...
                    {getCategoryInfo(selectedCategory)?.label}
                    <button onClick={() => handleCategoryChange('all')} className="text-blue-600 hover:text-blue-800">
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                )}
                {searchQuery && (
                  <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-purple-100 to-pink-100 text-purple-700 rounded-full text-sm font-medium">
                    ...
                    <button onClick={() => setSearchQuery('')} className="text-purple-600 hover:text-purple-800">
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                )}
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    {sortOptions.map(option => (
                      ...
                      >
                        {option.icon}<span className="text-sm">{option.label}</span>
                        {sortBy === option.value && <Check className="w-4 h-4 ml-auto text-blue-600" />}
                      </button>
                    ))}
                  </div>
                </div>
                
                <button onClick={clearFilters} className="w-full mt-6 py-3 bg-gradient-to-r from-gray-900 to-black text-white rounded-xl font-semibold hover:shadow-xl transition-all">
                  Clear All Filters
                </button>
              </div>
            </div>
          ...
          <div className={`${showFilters ? 'lg:w-3/4' : 'w-full'}`}>
              ...
              <div>
                <div className="flex items-center gap-4 mb-2">
                  ...
                  <div className="px-3 py-1 bg-gradient-to-r from-green-50 to-emerald-50 rounded-full">
                    <span className="text-sm font-bold text-green-700">{filteredProducts.length} items</span>
                  </div>
                </div>
                <p className="text-gray-600">
                  {selectedCategory === 'all' ? 'Complete collection of TV merchandise' : `Premium ${getCategoryInfo(selectedCategory)?.label.toLowerCase()} from top TV shows`}
                </p>
              </div>
              
              <div className="mt-4 lg:mt-0 flex items-center gap-4">
                <div className="hidden sm:flex items-center gap-2 bg-white rounded-xl p-1 border border-gray-200">
                  <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg ${viewMode === 'grid' ? 'bg-gradient-to-r from-blue-50 to-cyan-50 text-blue-600' : 'text-gray-600 hover:text-gray-900'}`}>
                    <LayoutGrid className="w-5 h-5" />
                  </button>
                  <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-gradient-to-r from-blue-50 to-cyan-50 text-blue-600' : 'text-gray-600 hover:text-gray-900'}`}>
                    <LayoutList className="w-5 h-5" />
                  </button>
                </div>
                
                <Link to="/" className="group inline-flex items-center gap-2 px-4 py-3 bg-gradient-to-r from-gray-900 to-black text-white rounded-xl font-bold hover:shadow-xl transition-all">
                  <ShoppingBag className="w-4 h-4" />Back to Home<ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </Link>
              </div>
            </div>
              ...
              {loading ? (
                <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl p-12 text-center mb-12">
                  ...
                  <p className="text-gray-600 mb-8 max-w-md mx-auto">
                    {selectedCategory !== 'all' ? `Fetching premium ${getCategoryInfo(selectedCategory)?.label.toLowerCase()}...` : 'Loading our complete merchandise collection'}
                  </p>
                </div>
                ...
                <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl p-12 text-center mb-12">
                  ...
                  <p className="text-gray-600 mb-8 max-w-md mx-auto">
                    {searchQuery ? `No products matching "${searchQuery}"` : `No products available in ${getCategoryInfo(selectedCategory)?.label}`}
                  </p>
                  {(searchQuery || selectedCategory !== 'all') && (
                    <button onClick={clearFilters} className="group inline-flex items-center gap-3 px-8 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all">
                      <ShoppingBag className="w-5 h-5" />Browse All Products<Zap className="w-4 h-4 group-hover:rotate-12 transition-transform" />
                    </button>
                  )}
                </div>
              ) : (
                <>
                  ...
                  {shouldShowViewAllButton && (
                    <div className="mt-12 text-center">
                      <button onClick={isMobile ? handleViewAllProducts : handleViewAllProductsPC} className="group inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-2xl font-bold shadow-xl hover:shadow-2xl transition-all transform hover:scale-105">
                        <Eye className="w-5 h-5" />View All Products ({filteredProducts.length})<ArrowRight className="w-5 h-5 group-hover:translate-x-2 transition-transform" />
                      </button>
                      <p className="text-gray-600 mt-4 text-sm">
                        {isMobile ? `Showing 4 of ${filteredProducts.length} products` : `Showing 8 of ${filteredProducts.length} products`}
                      </p>
                    </div>
                  )}
                </>
            ...
            <div className="mt-16">
              <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={() => { handleCategoryChange('all'); setTimeout(() => { if (productsSectionRef.current) productsSectionRef.current.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 100); }} className="text-blue-600 hover:text-blue-800 font-medium">
                  View All
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4">
                {categories.map((category) => (
                  <button key={category.value} onClick={() => handleCategoryChange(category.value)} className={`group relative bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border-2 h-40 sm:h-48 ${selectedCategory === category.value ? 'border-blue-500 shadow-blue-100' : 'border-transparent hover:border-blue-200'}`}>
                    <div className={`h-24 sm:h-28 ${category.bgGradient} flex flex-col items-center justify-center relative overflow-hidden`}>
                      <div className="text-2xl sm:text-3xl mb-1 transform group-hover:scale-110 transition-transform">{category.icon}</div>
                      ...
                      {selectedCategory === category.value && <div className="absolute top-3 right-3"><div className="w-3 h-3 bg-white rounded-full animate-ping"></div></div>}
                    </div>
                    <div className="p-3 sm:p-4">
                      ...
                      {selectedCategory === category.value && <div className="mt-2 flex justify-center"><div className="w-2 h-2 bg-blue-500 rounded-full"></div></div>}
                    </div>
                    {selectedCategory === category.value && <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 to-purple-500"></div>}
                  </button>
                ))}
              </div>
            </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    <div className="text-center sm:text-left">
                      <p className="text-gray-600">Showing <span className="font-bold text-blue-600">{filteredProducts.length}</span> products</p>
                      <p className="text-sm text-gray-500 mt-1">Sorted by <span className="font-semibold">{sortOptions.find(o => o.value === sortBy)?.label}</span></p>
                    </div>
                  ...
                  <div className="flex items-center gap-4">
                    <button onClick={handleRefresh} className="group flex items-center gap-2 text-blue-600 hover:text-blue-800">
                      <RefreshCw className="w-4 h-4 group-hover:rotate-180 transition-transform" /><span className="font-semibold">Refresh Results</span>
                    </button>
                    <Link to="/" className="group inline-flex items-center gap-2 px-4 py-3 bg-gradient-to-r from-gray-900 to-black text-white rounded-xl font-semibold hover:shadow-xl transition-all">
